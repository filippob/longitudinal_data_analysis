...
```{r}
autoplot(ts(co2, start=1980)) + ggtitle("Monthly atmospheric CO2 concentration in Manua Loa")
```
...
The horizontal lines indicate the means for each month. This plot highlights the underlying seasonal pattern to be seen clearly, and also shows the changes in seasonality over time.

Now we use data from two paired time series: electricity demand (Gigawatts) and temeprature (Celsius degrees) from Victoria (Australia) in 2014:
...
inpfile = "data/planktontimeseries/Acartia_spp_(unidentified).csv"
fname = file.path(basefolder, inpfile)

plankton = fread(fname, header = TRUE)
row.names(plankton) <- as.character(plankton$`0`)
# plankton <- select(plankton, -`0`)
plk = ts(t(plankton), start=1958, end=2013, names = plankton$`0`)
```
...
```{r}
GGally::ggpairs(as.data.frame(plk[,1:5]))
```
...
```{r}
acf(co2, lag = 3, plot = FALSE)
```

```{r}
acf(co2, lag.max = 8, plot = FALSE)
```
...
$ \pm \frac{2}{\sqrt{T}}$ where $T$ is the length of the time series. 
