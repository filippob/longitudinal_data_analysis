...
for (i in 1:nmonths) {
  ...
  training_observed = window(data, start = c(1949,1), end = c(1956,(12+i)), frequency = 12)
...
```{r}
temp$year <- NULL
temp = ts(temp, start = 1775, end = 2021)
```
...
```{r}
plot(data, col="blue", xlab="Year", ylab="Passengers", main="SARIMA Forecast", type='l')
lines(ts(one_step_ahead_sarima[,2], start = 1, frequency = 1), col="red", lwd=3)
```
