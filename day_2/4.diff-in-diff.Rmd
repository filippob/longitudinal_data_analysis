...
This dataset comes from the R package **wooldridge** ([here](https://cran.r-project.org/web/packages/wooldridge/index.html)).
...
This is shown graphically in the plot below: the dashed gray line is the **counterfactual**.
...
Rather than calculating diff-in-diff by hand, we can use a regression model which, besides simplifying the calculations,
will also allow for a more flxible, powerful and robust analysis (e.g. account for covariables).
...
We see that we got the same value for the coefficient of the interaction term, as we did by hand (ressuring ;-)).
