...
```{r}
fit = lm(value ~ Week + Treatment, data = school_time)
summary(fit)
```
...
Specific contrasts between levels of the categorical explanatory variables can be obtained with a number of ways.
E.g., buy using the `TukeyHSD()` function (e.g. [here](https://en.wikipedia.org/wiki/Tukey%27s_range_test)) which requires an `aov` object:
