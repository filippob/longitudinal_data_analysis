...
```{r}
res <- riskratio.wald(Table2, rev = c("both"))
...
```{r}
res <- riskratio.wald(Table3, rev = c("both"))
...
```{r}
res <- riskratio.wald(Table4, rev = c("both"))
...
```{r}
res <- riskratio.wald(Table5, rev = c("both"))
...
Compared to the crude analysis where the RR = 1.22 (and the OR = 1.25), the stratified results are much higher. t
...
It would be convenient to have a way to get an estimate of the X-Y association adjusted for confounding, without having to go through the complications of the stratified analysis: this is the **Mantel-Haenszel (M-H)** test/metric.
